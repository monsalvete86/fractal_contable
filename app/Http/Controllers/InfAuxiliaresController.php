...
class InfAuxiliaresController extends Controller
{
    ...
    public function AuxConSaldos(Request $request){
        ...
        $cons="SELECT cast(num_cuenta as integer) as num_cuenta,cuentas.fecha,nombre_formato,cuentas.numero ,cuentas.doc_afecta_long , cuentas.detalle, personas.nombre,nombre1,nombre2,apellido1,apellido2,num_documento,debe,haber , (debe - haber) as saldo, cuentas.id, plan_cuentas.nombre as nom_cuenta FROM cuentas,conf_formatos,formatos, personas, plan_cuentas WHERE formatos.condicion = 1 and cuentas.id_formato = formatos.id and formatos.formato = conf_formatos.id and plan_cuentas.id = cuentas.cuenta and personas.id = cuentas.tercero and cuentas.fecha >= '$fec_ini' and cuentas.fecha <='$fec_fin' $cuenta_ini $cuenta_fin $id_tercero order by num_cuenta asc, cuentas.fecha";
