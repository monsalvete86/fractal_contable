...
use App\Cuentas;
use Carbon\Carbon;
//include "..\..\..\vendor\barryvdh\laravel-dompdf\PDF.php";
...
class FormatoController extends Controller
{
    ...
    public function PdfAfiliados(Request $request, $id)
    {
  
        $cons="SELECT * FROM pagoslapaz WHERE id=$id";
        $pago = DB::select($cons); 
        $pago = $pago[0];

        $cons="SELECT numero,id,tercero,fecha,detalle FROM formatos WHERE id=".$pago->id_reg_cuenta;
        $recibo_caja = DB::select($cons); 
        $recibo_caja = $recibo_caja[0];

        $cons="SELECT nombre1,nombre2,apellido1,apellido2,num_documento, tipo_documento FROM personas WHERE id=".$recibo_caja->tercero;
        $tercero = DB::select($cons); 
        $tercero = $tercero[0];

        //echo "<pre>"; print_r($recibo_caja); echo "</pre>";
        $formato="";    $detalles="";
        return view('pdf.reciboPago')->with('recibo_caja', $recibo_caja)->with('pago', $pago)->with('tercero', $tercero);
        

       // $pdf = \PDF::loadView('pdf.reciboPago',['recibo_caja'=>$recibo_caja,'pago'=>$pago,'tercero'=>$tercero]);
        
        //return $pdf->download('recibo-'.$recibo_caja->numero.'.pdf');
